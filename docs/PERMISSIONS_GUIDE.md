...
```
/setup ai <set-model | set-primary | reset-model | status>
/setup knowledge <add-channel | remove-channel | add-message | remove-message | add-text | remove-text | reset | status>
/setup notify <set | reset | status>
...
- **`Administrator` (Quản trị viên máy chủ)**: Có toàn quyền tuyệt đối đối với tất cả lệnh `/setup`.
- **`ManageGuild` (Quản lý máy chủ)**: Có quyền cấu hình AI Model (`ai`), Kênh Tri Thức (`knowledge`), Kênh thông báo (`notify`), Bật/Tắt module (`feature`), Kênh lệnh (`channel`) và Danh sách trắng (`whitelist`).
...
+---------------------------+-----------------------+-----------------------------+
| Cấu hình AI Model         | /setup ai ...         | Manage Server (ManageGuild) |
| Cấu hình Kênh Tri Thức AI | /setup knowledge ...  | Manage Server (ManageGuild) |
| Cấu hình Kênh Thông Báo   | /setup notify ...     | Manage Server (ManageGuild) |
| Cấu hình Kênh Lệnh        | /setup channel ...    | Manage Channels / Server    |
| Quản lý Whitelist         | /setup whitelist ...  | Manage Messages / Server    |
| Bật/Tắt Tính Năng         | /setup feature ...    | Manage Server (ManageGuild) |
| Hỏi đáp AI Assistant      | /ask ... / !ask ...   | @everyone (Rate limit 5s)   |
